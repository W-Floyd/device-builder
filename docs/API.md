...
| `already_exists` | Resource already exists (duplicate add / concurrent rotate) |
...
| `unavailable` | A *remote* dashboard couldn't be reached (TLS handshake failed, host refused, timed out). Distinct from `internal_error`, which is a fault on *this* dashboard. |
| `unauthorized` | A *remote* dashboard rejected our bearer (wrong token, revoked, or 403 because token is bound to a different dashboard). Distinct from `not_authenticated`, which is about the client's WS auth gate to this dashboard. |
| `precondition_failed` | A pre-condition the caller asserted no longer holds — currently used only by `remote_build/confirm_pair` when the re-handshake's pin doesn't match the user-confirmed value. |
...
| `remote_build/preview_pair` | `{hostname, port}` | `PairingPreview` | Phase 1 of the offloader-side pair flow (phase 4a). Opens a TLS handshake to the candidate receiver, returns `{pin_sha256}` (the SPKI fingerprint observed during the handshake). No bearer presented, no persistence; the frontend renders the fingerprint for the user to confirm out-of-band against what the receiver's Build server settings page shows. `unavailable` if the host is unreachable / handshake times out; `invalid_args` on bad hostname / port. |
| `remote_build/confirm_pair` | `{hostname, port, pin_sha256, token, label, server_version?, esphome_version?}` | `PairingSummary` | Phase 2 of the pair flow. Re-handshakes (defends against a TOCTOU between preview and confirm), asserts the new handshake's pin matches `pin_sha256`, presents `token` as `Authorization: Bearer …` to the receiver's `GET /remote-build/v1/health` with this dashboard's stable `X-Dashboard-ID`, and persists the `StoredPairing` only when both succeed. `precondition_failed` on pin mismatch (cert rotated under us or active MITM); `unauthorized` on bearer rejection (wrong / revoked / wrong-binding); `unavailable` on transport failure; `already_exists` if `(hostname, port)` is already paired (call `unpair` first); `invalid_args` if the offloader pool is at the 100-entry cap. The bearer is **sealed** with a per-installation Fernet key (kept at `<config_dir>/.device-builder-offload-key.bin` with 0o600 perms, lazy-created on first pair) before it lands on disk; phase 5 unseals once per outbound peer-link request. |
| `remote_build/unpair` | `{hostname, port}` | `RemoteBuildSettingsView` | Remove a paired-remote record by `(hostname, port)`. `not_found` if no match. The receiver still has the bound token until it's revoked there separately — unpair is local to the offloader. |
| `remote_build/list_pool` | — | `[PairingSummary]` | Return the offloader's current pool of paired remotes. Wire view drops `token_sealed`; the offloader keeps the encrypted bearer on disk to authenticate peer-link requests, but it never reaches the frontend even in sealed form. |
